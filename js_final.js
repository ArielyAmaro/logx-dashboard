...
function popularSelects(){
  var mS={},mSetd={},aS={},oS={},dS={},sS={},mrcS={};
  RAW.forEach(function(r){
    if(r.mes&&r.ano){var k=r.ano+'-'+('0'+r.mes).slice(-2);mS[k]=1;}
    if(r.mes_etd&&r.ano_etd){var ke=r.ano_etd+'-'+('0'+r.mes_etd).slice(-2);mSetd[ke]=1;}
  ...
  Object.keys(mS).sort().forEach(function(k){
    var p=k.split('-');
    add('fmes',p[1],MP[+p[1]]+'/'+p[0]);
  });
  /* dmes/dmes2 usam ETD */
  Object.keys(mSetd).sort().forEach(function(k){
    var p=k.split('-');
    add('dmes',k,MP[+p[1]]+'/'+p[0]);add('dmes2',k,MP[+p[1]]+'/'+p[0]);
  });
  Object.keys(aS).sort().forEach(function(a){
    add('farm',a,a);add('pfarm',a,a);add('varm',a,a);add('darm',a,a);
  });
  Object.keys(oS).sort().forEach(function(a){add('fori',a,a);});
  Object.keys(dS).sort().forEach(function(a){add('fdest',a,a);add('vdest',a,a);add('ddest',a,a);});
  ...
  var ksEtd=Object.keys(mSetd).sort();
  var dEl=document.getElementById('dmes');
  if(dEl&&ksEtd.length>0)dEl.value=ksEtd[ksEtd.length-1];
}
...
function render(){
  ...
  rSmpClickable(data,'exportador','tbl-exp',8);
  rSmpClickable(data,'cliente','tbl-cli',8);
...
/* --- MENSAL com cancelados --- */
function rMensal(data,f){
  var d=fcl(data);
  var dCan=data.filter(function(r){return r.fcl==='FCL'&&r.situacao==='Cancelado';});
  var maxV=0;
  for(var m=1;m<=12;m++){
  ...
  for(var m=1;m<=12;m++){
    (function(mm){
      ...
      var canMes=dCan.filter(function(r){return r.mes===mm;});
      var w25=Math.round(v25/maxV*100),w26=Math.round(v26/maxV*100);
      var canInfo='';
      if(canMes.length){
        var arms={},clis={};
        canMes.forEach(function(r){
          if(r.armador)arms[r.armador]=(arms[r.armador]||0)+1;
          if(r.cliente)clis[r.cliente]=(clis[r.cliente]||0)+1;
        });
        var topArm=Object.entries(arms).sort(function(a,b){return b[1]-a[1];}).map(function(e){return e[0].split(' ')[0];}).slice(0,2).join(', ');
        var topCli=Object.entries(clis).sort(function(a,b){return b[1]-a[1];}).map(function(e){return e[0].length>18?e[0].slice(0,16)+'...':e[0];}).slice(0,1).join('');
        canInfo='<span class="can-tag" title="'+canMes.length+' cancelado(s) | Arm: '+topArm+(topCli?' | Cli: '+topCli:'')+'">'
          +'<span style="color:#ff5555;font-size:9px;font-weight:700;">'+canMes.length+' CAN</span>'
          +'</span>';
      }
      if(!f.ano){
        h+='<div class="brow">'
          ...
          +'</div>'
          +(canInfo?'<div style="margin-left:4px;display:flex;align-items:center;">'+canInfo+'</div>':'')
          +'</div>';
      }else{
...
function rTopArm(data){
  ...
  top.forEach(function(e,i){
    ...
    h+='<div class="arm-click-row" data-arm="'+i+'" style="display:flex;align-items:center;gap:7px;font-size:11px;cursor:pointer;" title="Ver rotas de '+nm+'">'
      +'<div style="flex:1;color:#fff;min-width:0;">'+sh+'</div>'
      ...
      +'<div style="color:#fff;width:34px;text-align:right;flex-shrink:0;">'+fmt(v)+'</div>'
      +'<div style="color:#fff;font-size:10px;flex-shrink:0;">\u203a</div></div>';
  });
  h+='</div>';
  var el=document.getElementById('chart-arm');
  el.innerHTML=h;
  var _top=top;
  el.querySelectorAll('.arm-click-row').forEach(function(row){
    row.addEventListener('click',function(){
      var idx=+row.getAttribute('data-arm');
      abrirDetArm(_top[idx][0],data);
    });
    row.addEventListener('mouseenter',function(){row.style.background='rgba(202,255,0,.04)';});
    row.addEventListener('mouseleave',function(){row.style.background='';});
  });
}

/* --- ARMADORES OVERLAY --- */
function abrirDetArm(nomeArm,data){
  var d=fcl(data).filter(function(r){return r.armador===nomeArm;});
  var t25=0,t26=0;
  d.forEach(function(r){if(r.ano===2025)t25+=r.teus;else if(r.ano===2026)t26+=r.teus;});
  var tot=t25+t26;
  var vp=pct(t25,t26);
  document.getElementById('det-nm').textContent=nomeArm;
  document.getElementById('det-sub').textContent='Armador \u00b7 '+d.length+' processos FCL';
  ...
  d.forEach(function(r){
    ...
    if(!mRota[k])mRota[k]={teus:0,cnt:0};
    mRota[k].teus+=r.teus;mRota[k].cnt+=1;
  });
  var rotas=Object.entries(mRota).sort(function(a,b){return b[1].teus-a[1].teus;}).slice(0,12);
  ...
  rotas.forEach(function(e){
    ...
    lRota+='<div style="padding:6px 0;border-bottom:1px solid #111;">'
      ...
      +'<div style="font-size:10px;color:#555;">'+info.cnt+' proc</div>'
      +'<div style="font-size:11px;color:#CAFF00;font-weight:700;">'+fmt(info.teus)+' TEUs</div></div>'
      +'<div style="flex:1;height:3px;background:#111;border-radius:2px;overflow:hidden;">'
      +'<div style="width:'+w+'%;height:100%;background:#CAFF00;"></div></div></div>';
  });

  var mCli={};
  d.forEach(function(r){if(r.cliente)mCli[r.cliente]=(mCli[r.cliente]||0)+r.teus;});
  var clis=Object.entries(mCli).sort(function(a,b){return b[1]-a[1];}).slice(0,8);
  var maxC=clis.length?clis[0][1]:1;
  var lCli='';
  clis.forEach(function(e,i){
    var nm=e[0],v=e[1];
    var w=Math.round(v/maxC*100);
    var p2=Math.round(v/tot*100);
    var cor=MC[i%MC.length];
    var sh=nm.length>24?nm.slice(0,22)+'...':nm;
    lCli+='<div style="margin-bottom:8px;">'
  ...
  document.getElementById('det-body').innerHTML=
    '<div class="dcol"><div class="dctitle">Rotas operadas</div>'+lRota+'</div>'
    +'<div class="dcol"><div class="dctitle">Principais clientes</div>'+lCli+'</div>';
...
function rTblArm(data,f){
  ...
  rows.forEach(function(r,i){
    h+='<tr class="arm-tbl-row" data-arm-idx="'+i+'" style="cursor:pointer;" title="Ver rotas de '+r.nm+'">'
      +'<td><span class="nm">'+r.nm+'</span></td>'
      +'<td class="r">'+(r.v25?fmt(r.v25):'-')+'</td>'
      +'<td class="r">'+(r.v26?fmt(r.v26):'-')+'</td>'
      +'<td class="r">'+cv(pct(r.v25,r.v26))+'</td></tr>';
  });
  var el=document.getElementById('tbl-arm');
  el.innerHTML=h+'</tbody>';
  var _rows=rows;var _data=data;
  el.querySelectorAll('.arm-tbl-row').forEach(function(row){
    row.addEventListener('click',function(){
      var idx=+row.getAttribute('data-arm-idx');
      abrirDetArm(_rows[idx].nm,_data);
    });
    row.addEventListener('mouseenter',function(){row.style.background='rgba(202,255,0,.04)';});
    row.addEventListener('mouseleave',function(){row.style.background='';});
  });
...
function rMerc(data){
  ...
  top.forEach(function(e,i){
    ...
    var cor=MC[i]||'#555';
    h+='<div class="mi merc-click" data-merc-idx="'+i+'" style="cursor:pointer;" title="Ver armadores e rotas">'
      ...
      +'<div class="mpct">'+p+'%</div>'
      +'<div style="color:#fff;font-size:10px;margin-left:2px;">\u203a</div>'
      +'</div>';
  });
  var el=document.getElementById('list-merc');
  el.innerHTML=h;
  var _top=top;var _data=data;
  el.querySelectorAll('.merc-click').forEach(function(row){
    row.addEventListener('click',function(){
      var idx=+row.getAttribute('data-merc-idx');
      abrirDetMerc(_top[idx][0],_data);
    });
...
/* --- MERCADORIA OVERLAY --- */
function abrirDetMerc(nomeMerc,data){
  var d=fcl(data).filter(function(r){return r.mercadoria===nomeMerc;});
  var t25=0,t26=0;
  d.forEach(function(r){if(r.ano===2025)t25+=r.teus;else if(r.ano===2026)t26+=r.teus;});
  var tot=t25+t26;
  var vp=pct(t25,t26);
  document.getElementById('det-nm').textContent=nomeMerc;
  document.getElementById('det-sub').textContent='Mercadoria \u00b7 '+d.length+' processos FCL';
  document.getElementById('det-kpis').innerHTML=
    '<div class="dk"><div class="dkl">Total TEUs</div><div class="dkv ac">'+fmt(tot)+'</div></div>'
    +'<div class="dk"><div class="dkl">TEUs 2025</div><div class="dkv">'+fmt(t25)+'</div></div>'
    +'<div class="dk"><div class="dkl">TEUs 2026</div><div class="dkv">'+fmt(t26)+'</div></div>'
  ...
  var mArm={};
  d.forEach(function(r){if(r.armador)mArm[r.armador]=(mArm[r.armador]||0)+r.teus;});
  ...
  rotas.forEach(function(e){
    var rota=e[0],info=e[1];
    var w=Math.round(info.teus/maxR*100);
    var topA=d.filter(function(r){return ((r.origem||'-')+' \u2192 '+(r.destino||'-'))===rota;});
    var armMap={};topA.forEach(function(r){if(r.armador)armMap[r.armador]=(armMap[r.armador]||0)+1;});
    var mainArm=Object.entries(armMap).sort(function(a,b){return b[1]-a[1];})[0];
    var aN=mainArm?mainArm[0].split(' ')[0]:'';
    lRota+='<div style="padding:6px 0;border-bottom:1px solid #111;">'
      +'<div style="display:flex;align-items:center;gap:7px;margin-bottom:3px;">'
      +'<div style="flex:1;font-size:11px;color:#ccc;">'+rota+'</div>'
      +'<div style="font-size:10px;color:#555;">'+info.cnt+' proc</div>'
      ...
      +(aN?'<div style="font-size:9px;color:#333;white-space:nowrap;">via '+aN+'</div>':'')
      +'</div></div>';
  });

  document.getElementById('det-body').innerHTML=
    '<div class="dcol"><div class="dctitle">Armadores</div>'+lArm+'</div>'
    +'<div class="dcol"><div class="dctitle">Rotas (Origem \u2192 Destino)</div>'+lRota+'</div>';
  document.getElementById('overlay').classList.add('on');
}

/* --- SMP CLICKAVEL (exportador/cliente na visao geral) --- */
function rSmpClickable(data,campo,elId,top){
  ...
  rows.forEach(function(r,i){
    ...
    h+='<tr class="smp-click" data-idx="'+i+'" style="cursor:pointer;" title="Ver rotas e armadores">'
      +'<td><span class="nm" title="'+r.nm+'">'+nm+' \u203a</span></td>'
      +'<td class="r">'+(r.v25?fmt(r.v25):'-')+'</td>'
  ...
  el.innerHTML=h+'</tbody>';
  var _rows=rows;
  el.querySelectorAll('.smp-click').forEach(function(row){
    row.addEventListener('click',function(){
      var idx=+row.getAttribute('data-idx');
      abrirDet(campo,_rows[idx].nm);
    });
    row.addEventListener('mouseenter',function(){row.style.background='rgba(202,255,0,.04)';});
    row.addEventListener('mouseleave',function(){row.style.background='';});
  });
...
function buscar(){
  ...
  if(hits.length>3){
    html+='<div style="font-size:10px;color:#fff;text-align:center;padding:8px;">+'+(hits.length-3)+' processos - refine a busca</div>';
  }
  res.innerHTML=html;
}
...
function rLista(campo,listId,txtId,anoId,mesId){
  var txt=(document.getElementById(txtId).value||'').toLowerCase();
  var ano=document.getElementById(anoId).value;
  var mes=mesId&&document.getElementById(mesId)?document.getElementById(mesId).value:'';
  var m={};
  RAW.filter(function(r){
    return okFCL(r)&&r[campo]
      &&(!ano||String(r.ano)===ano)
      &&(!mes||String(r.mes)===mes)
      &&(!txt||r[campo].toLowerCase().indexOf(txt)>=0);
  }).forEach(function(r){
    if(!m[r[campo]])m[r[campo]]={teus:0,proc:0,p25:0,p26:0};
    m[r[campo]].teus+=r.teus;
    m[r[campo]].proc+=1;
    if(r.ano===2025)m[r[campo]].p25+=1;
    else if(r.ano===2026)m[r[campo]].p26+=1;
  });
  var rows=Object.entries(m).sort(function(a,b){return b[1].teus-a[1].teus;}).slice(0,30);
  var max=rows.length?rows[0][1].teus:1;
  var h='';
  h+='<div style="display:flex;align-items:center;gap:6px;padding:0 8px 6px;border-bottom:1px solid #1a1a1a;margin-bottom:2px;">'
    +'<div style="width:19px;flex-shrink:0;"></div>'
    +'<div style="flex:1;font-size:8px;color:#fff;text-transform:uppercase;letter-spacing:.5px;">Nome</div>'
    +'<div style="width:65px;font-size:8px;color:#fff;text-transform:uppercase;flex-shrink:0;">TEUs</div>'
    +(ano?'':'<div style="width:68px;text-align:center;font-size:8px;flex-shrink:0;"><span style="color:#4a9eff;">\u002725</span> <span style="color:#555;">vs</span> <span style="color:#CAFF00;">\u002726</span></div>')
    +'<div style="width:38px;text-align:right;font-size:8px;color:#fff;text-transform:uppercase;flex-shrink:0;">Proc.</div>'
    +'<div style="width:14px;flex-shrink:0;"></div>'
    +'</div>';
  rows.forEach(function(e,i){
    var nm=e[0],d=e[1];
    var w=Math.round(d.teus/max*100);
    var cor=MC[i%MC.length];
    var sh=nm.length>32?nm.slice(0,30)+'...':nm;
    var compHtml='';
    if(!ano){
      var tot2=d.p25+d.p26||1;
      var w25=Math.round(d.p25/tot2*100);
      var w26=100-w25;
      compHtml='<div style="width:68px;flex-shrink:0;display:flex;flex-direction:column;gap:2px;justify-content:center;">'
        +'<div style="display:flex;height:5px;border-radius:2px;overflow:hidden;background:#111;">'
        +'<div style="width:'+w25+'%;background:#4a9eff;"></div>'
        +'<div style="width:'+w26+'%;background:#CAFF00;"></div>'
        +'</div>'
        +'<div style="display:flex;justify-content:space-between;font-size:8px;color:#888;">'
        +'<span>'+d.p25+'</span><span>'+d.p26+'</span>'
        +'</div></div>';
    }
    h+='<div class="eitem" data-campo="'+campo+'" data-idx="'+i+'" style="padding:5px 8px;border-bottom:1px solid #111;">'
      ...
      +'<div style="width:'+w+'%;height:100%;background:'+cor+';border-radius:2px;"></div></div>'
      +'<div style="font-size:9px;color:#fff;flex-shrink:0;min-width:32px;text-align:right;">'+fmt(d.teus)+'</div>'
      +'</div></div>'
      +compHtml
      +'<div style="width:38px;text-align:right;font-size:11px;color:#fff;font-weight:700;flex-shrink:0;">'+d.proc+'</div>'
      +'<div class="earr">\u203a</div></div>';
  });
  var container=document.getElementById(listId);
  container.innerHTML=h||'<div style="color:#fff;padding:16px;text-align:center;">Nenhum resultado</div>';
  var _rows=rows;
...
function rExpList(){rLista('exportador','list-exp','exp-txt','exp-ano','exp-mes');}
function rCliList(){rLista('cliente','list-cli','cli-txt','cli-ano','cli-mes');}
...
function abrirDet(campo,nome){
  ...
  rotas.forEach(function(e){
    ...
    lRota+='<div style="padding:6px 0;border-bottom:1px solid #111;">'
      ...
      +'<div style="flex:1;height:3px;background:#111;border-radius:2px;overflow:hidden;">'
      +'<div style="width:'+w+'%;height:100%;background:#CAFF00;"></div></div>'
      +'<div style="font-size:9px;color:#fff;white-space:nowrap;">via '+aN+'</div></div></div>';
  });
...
/* ===================== DATAMAR VIEW ===================== */
function renderDatamar(){
  var modo=document.getElementById('dmodo')?document.getElementById('dmodo').value:'mes';
  var isMes=modo==='mes';

  /* toggle visibility of mes vs ano selectors */
  ['fg-mes1','fg-mes2'].forEach(function(id){var e=document.getElementById(id);if(e)e.style.display=isMes?'':'none';});
  ['fg-ano1','fg-ano2'].forEach(function(id){var e=document.getElementById(id);if(e)e.style.display=isMes?'none':'';});

  var armv=document.getElementById('darm').value;
  var destv=document.getElementById('ddest').value;

  var base=RAW.filter(function(r){
    return okFCL(r)&&(!armv||r.armador===armv)&&(!destv||r.destino===destv);
  });

  var p1=null,p2=null,hasComp=false,lb1='Todos periodos',lb2='';

  if(isMes){
    var mesv=document.getElementById('dmes').value;
    var mes2v=document.getElementById('dmes2').value;
    function pmM(v){if(!v)return null;var p=v.split('-');return{ano:+p[0],mes:+p[1]};}
    p1=pmM(mesv);p2=pmM(mes2v);hasComp=!!(p1&&p2);
    lb1=p1?(MP[p1.mes]+'/'+p1.ano):'Todos';
    lb2=p2?(MP[p2.mes]+'/'+p2.ano):'';
  } else {
    var ano1v=document.getElementById('dano1')?document.getElementById('dano1').value:'';
    var ano2v=document.getElementById('dano2')?document.getElementById('dano2').value:'';
    if(ano1v)p1={ano:+ano1v,mes:null};
    if(ano2v)p2={ano:+ano2v,mes:null};
    hasComp=!!(p1&&p2);
    lb1=ano1v||'Todos';
    lb2=ano2v||'';
  }
  ...
  function getD(p){
    if(!p)return base;
    if(isMes) return base.filter(function(r){return r.ano_etd===p.ano&&r.mes_etd===p.mes;});
    return base.filter(function(r){return r.ano_etd===p.ano;});
  }
  var d1=getD(p1);
  var d2=getD(p2);

  var teus1=d1.reduce(function(s,r){return s+r.teus;},0);
  var teus2=d2.reduce(function(s,r){return s+r.teus;},0);
  var proc1=d1.length;
  var proc2=d2.length;
  var arms1=Object.keys(d1.reduce(function(m,r){if(r.armador)m[r.armador]=1;return m;},{})).length;
  var dests1=Object.keys(d1.reduce(function(m,r){if(r.destino)m[r.destino]=1;return m;},{})).length;
  var vpT=hasComp?pct(teus1,teus2):null;
  var vpP=hasComp?pct(proc1,proc2):null;

  /* -- label do cabecalho -- */
  var hdrEl=document.getElementById('dat-hdr');
  if(hdrEl){
    hdrEl.innerHTML='<span style="font-size:10px;text-transform:uppercase;letter-spacing:2px;color:#CAFF00;font-weight:700;">KPIs por ETD / Data de Embarque</span>'
      +(hasComp?'<span style="font-size:9px;color:#fff;margin-left:10px;border:1px solid #2a2a2a;padding:3px 8px;border-radius:2px;">'
        +'<span style="color:#4a9eff;">'+lb1+'</span>'
        +' <span style="color:#555;">vs</span> '
        +'<span style="color:#CAFF00;">'+lb2+'</span></span>'
        :'<span style="font-size:9px;color:#fff;margin-left:8px;">Processos FCL por '+( isMes?'mes':'ano' )+' de partida</span>');
  }

  function kd(cl,lbl,val,sup,sub){
    return '<div class="kpi '+cl+'">'
      +'<div class="klbl">'+lbl+'</div>'
      +'<div class="kval">'+val+(sup?'<sup>'+sup+'</sup>':'')+'</div>'
      +'<div class="ksub neu">'+sub+'</div></div>';
  }

  var kpis='';
  kpis+=kd('kb','TEUs '+lb1,fmt(teus1),'','base');
  if(hasComp){
    kpis+=kd(vpT>=0?'kg':'kr','TEUs '+lb2,fmt(teus2),'',
      (teus2>=teus1?'+':'')+fmt(teus2-teus1)+' TEUs');
    kpis+=kd(vpT>=0?'kg':'kr','Variacao TEUs',
      vpT!==null?(vpT>=0?'+':'')+vpT+'%':'N/D','',isMes?'mes vs mes':'ano vs ano');
    kpis+=kd('kb','Proc. '+lb1,proc1,'','base');
    kpis+=kd(vpP>=0?'kg':'kr','Proc. '+lb2,proc2,'',
      (proc2>=proc1?'+':'')+String(proc2-proc1)+' proc');
    kpis+=kd(vpP>=0?'kg':'kr','Variacao Proc.',
      vpP!==null?(vpP>=0?'+':'')+vpP+'%':'N/D','',isMes?'mes vs mes':'ano vs ano');
  }else{
    kpis+=kd('ka','Em aberto',d1.filter(function(r){return ['Embarcado','Aguardando embarque','Aguardando prontid\u00e3o da mercadoria'].indexOf(r.situacao)>=0;}).length,'','processos ativos');
    kpis+=kd('kb','Processos',proc1,'','embarques no periodo');
    kpis+=kd('kz','Armadores',arms1,'','no periodo');
    kpis+=kd('kz','Destinos',dests1,'','rotas ativas');
  }
  document.getElementById('dat-kpis').innerHTML=kpis;

  /* --- tabelas comparativas de armadores --- */
  function buildArmMap(d){
    var m={};
    d.forEach(function(r){if(r.armador){if(!m[r.armador])m[r.armador]={teus:0,proc:0};m[r.armador].teus+=r.teus;m[r.armador].proc++;}});
    return m;
  }
  var aMap1=buildArmMap(d1);
  var aMap2=hasComp?buildArmMap(d2):{};
  var allArms=Object.keys(Object.assign({},aMap1,aMap2));
  var armRows=allArms.map(function(k){
    return{nm:k,t1:(aMap1[k]||{teus:0}).teus,p1:(aMap1[k]||{proc:0}).proc,
           t2:(aMap2[k]||{teus:0}).teus,p2:(aMap2[k]||{proc:0}).proc};
  }).sort(function(a,b){return (b.t1+b.t2)-(a.t1+a.t2);}).slice(0,10);
  var maxAT=armRows.length?Math.max.apply(null,armRows.map(function(r){return Math.max(r.t1,r.t2);})):1;
  var hArm='<thead><tr><th>Armador</th>'
    +'<th class="r" style="color:#4a9eff;">'+lb1+'</th>'
    +(hasComp?'<th class="r" style="color:#CAFF00;">'+lb2+'</th><th class="r">Var%</th>':'')
    +'<th class="r"></th>'
    +'</tr></thead><tbody>';
  armRows.forEach(function(e){
    var sh=e.nm.length>22?e.nm.slice(0,20)+'...':e.nm;
    var vp2=hasComp?pct(e.t1,e.t2):null;
    var w1=Math.round(e.t1/maxAT*100);
    var w2=hasComp?Math.round(e.t2/maxAT*100):0;
    hArm+='<tr><td><span class="nm" title="'+e.nm+'">'+sh+'</span>'
      +'<div style="height:3px;background:#111;border-radius:1px;margin-top:3px;overflow:hidden;display:flex;gap:1px;">'
      +'<div style="width:'+w1+'%;height:100%;background:#4a9eff;border-radius:1px;"></div>'
      +(hasComp?'</div><div style="height:3px;background:#111;border-radius:1px;margin-top:1px;overflow:hidden;">'
        +'<div style="width:'+w2+'%;height:100%;background:#CAFF00;border-radius:1px;"></div>':'')
      +'</div></td>'
      +'<td class="r" style="color:#4a9eff;font-weight:700;">'+(e.t1?fmt(e.t1):'-')+'</td>'
      +(hasComp?'<td class="r" style="color:#CAFF00;font-weight:700;">'+(e.t2?fmt(e.t2):'-')+'</td>'
        +'<td class="r">'+cv(vp2)+'</td>':'')
      +'<td class="r"><button class="ibtn-sm dat-arm-btn" data-arm="'+e.nm+'" style="font-size:9px;padding:2px 7px;white-space:nowrap;">\u21b3 Destinos</button></td>'
      +'</tr>';
  });
  document.getElementById('dat-arm').innerHTML=hArm+'</tbody>';

  /* bind botoes de destinos nos armadores do datamar */
  document.querySelectorAll('.dat-arm-btn').forEach(function(btn){
    btn.addEventListener('click',function(ev){
      ev.stopPropagation();
      var nomeArm=this.getAttribute('data-arm');
      abrirDetArm(nomeArm,base);
    });
  });

  /* --- destinos comparativos --- */
  function buildDestMap(d){
    var m={};
    d.forEach(function(r){if(r.destino){if(!m[r.destino])m[r.destino]={teus:0,proc:0};m[r.destino].teus+=r.teus;m[r.destino].proc++;}});
    return m;
  }
  var dMap1=buildDestMap(d1);
  var dMap2=hasComp?buildDestMap(d2):{};
  var allDests=Object.keys(Object.assign({},dMap1,dMap2));
  var destRows=allDests.map(function(k){
    return{nm:k,t1:(dMap1[k]||{teus:0}).teus,t2:(dMap2[k]||{teus:0}).teus};
  }).sort(function(a,b){return (b.t1+b.t2)-(a.t1+a.t2);}).slice(0,10);
  var maxDT=destRows.length?Math.max.apply(null,destRows.map(function(r){return Math.max(r.t1,r.t2);})):1;
  var hDest='<thead><tr><th>Destino</th>'
    +'<th class="r" style="color:#4a9eff;">'+lb1+'</th>'
    +(hasComp?'<th class="r" style="color:#CAFF00;">'+lb2+'</th><th class="r">Var%</th>':'')
    +'</tr></thead><tbody>';
  destRows.forEach(function(e){
    var vp2=hasComp?pct(e.t1,e.t2):null;
    var w1=Math.round(e.t1/maxDT*100);
    var w2=hasComp?Math.round(e.t2/maxDT*100):0;
    hDest+='<tr><td><span class="nm">'+e.nm+'</span>'
      +'<div style="height:3px;background:#111;border-radius:1px;margin-top:3px;overflow:hidden;">'
      +'<div style="width:'+w1+'%;height:100%;background:#4a9eff;border-radius:1px;"></div>'
      +(hasComp?'</div><div style="height:3px;background:#111;border-radius:1px;margin-top:1px;overflow:hidden;">'
        +'<div style="width:'+w2+'%;height:100%;background:#CAFF00;border-radius:1px;"></div>':'')
      +'</div></td>'
  ...
  document.getElementById('dat-dest').innerHTML=hDest+'</tbody>';

  /* --- exportadores comparativos --- */
  function buildExpMap(d){
    var m={};
    d.forEach(function(r){if(r.exportador){if(!m[r.exportador])m[r.exportador]={teus:0,proc:0};m[r.exportador].teus+=r.teus;m[r.exportador].proc++;}});
    return m;
  }
  var eMap1=buildExpMap(d1);
  var eMap2=hasComp?buildExpMap(d2):{};
  var allExps=Object.keys(Object.assign({},eMap1,eMap2));
  var expRows=allExps.map(function(k){
    return{nm:k,t1:(eMap1[k]||{teus:0}).teus,t2:(eMap2[k]||{teus:0}).teus};
  }).sort(function(a,b){return (b.t1+b.t2)-(a.t1+a.t2);}).slice(0,8);
  var hExp='<thead><tr><th>Exportador</th>'
    +'<th class="r" style="color:#4a9eff;">'+lb1+'</th>'
    +(hasComp?'<th class="r" style="color:#CAFF00;">'+lb2+'</th><th class="r">Var%</th>':'')
    +'</tr></thead><tbody>';
  expRows.forEach(function(e){
    var sh=e.nm.length>26?e.nm.slice(0,24)+'...':e.nm;
    var vp2=hasComp?pct(e.t1,e.t2):null;
    hExp+='<tr><td><span class="nm" title="'+e.nm+'">'+sh+'</span></td>'
      +'<td class="r" style="color:#4a9eff;">'+(e.t1?fmt(e.t1):'-')+'</td>'
      +(hasComp?'<td class="r" style="color:#CAFF00;">'+(e.t2?fmt(e.t2):'-')+'</td>'
        +'<td class="r">'+cv(vp2)+'</td>':'')
      +'</tr>';
  });
  document.getElementById('dat-exp').innerHTML=hExp+'</tbody>';

  /* --- evolucao grafico --- */
  var anos=isMes?[2025,2026]:[2025,2026];
  var meses=[];
  if(isMes){
    for(var ay=2025;ay<=2026;ay++){for(var am=1;am<=12;am++)meses.push([ay,am]);}
  } else {
    anos.forEach(function(ay){meses.push([ay,null]);});
  }
  var maxMT=0;
  var mesVals=meses.map(function(am){
    var t;
    if(isMes){
      t=base.filter(function(r){return r.ano_etd===am[0]&&r.mes_etd===am[1];}).reduce(function(s,r){return s+r.teus;},0);
    } else {
      t=base.filter(function(r){return r.ano_etd===am[0];}).reduce(function(s,r){return s+r.teus;},0);
    }
    if(t>maxMT)maxMT=t;
    return{ano:am[0],mes:am[1],teus:t};
  });
  if(!maxMT)maxMT=1;
  var hEvo='';
  mesVals.forEach(function(mv){
    if(!mv.teus)return;
    var w=Math.round(mv.teus/maxMT*100);
    var hl1,hl2;
    if(isMes){
      hl1=p1&&p1.ano===mv.ano&&p1.mes===mv.mes;
      hl2=p2&&p2.ano===mv.ano&&p2.mes===mv.mes;
    } else {
      hl1=p1&&p1.ano===mv.ano;
      hl2=p2&&p2.ano===mv.ano;
    }
    var hl=hl1||hl2;
    var cor=hl1?'#4a9eff':hl2?'#CAFF00':mv.ano===2025?'#2f5a8a':'#587a00';
    var lbl=isMes?(MP[mv.mes]+'/'+String(mv.ano).slice(2)):String(mv.ano);
    hEvo+='<div class="brow">'
      +'<div class="blbl" style="width:'+(isMes?'44px':'36px')+';'+(hl?'color:#fff;font-weight:800;':'')+'">'+lbl+'</div>'
      +'<div class="bwrap"><div class="bsr"><div class="btr">'
      +'<div class="bf" style="width:'+w+'%;background:'+cor+';opacity:'+(hl?'1':'.35')+'"></div>'
      +'</div><div class="bnum" style="'+(hl?'color:#fff;font-weight:800;':'')+'">'+fmt(mv.teus)+'</div></div></div></div>';
  });
  document.getElementById('dat-evo').innerHTML=hEvo;
}
...
function aba(pg,el){
  ...
  if(pg==='dat')renderDatamar();
}

(function(){
  var maxTs=0;
  RAW.forEach(function(r){
    if(r.abertura){var p=r.abertura.split('/');if(p.length===3){var ts=new Date(+p[2],+p[1]-1,+p[0]).getTime();if(ts>maxTs)maxTs=ts;}}
  });
  document.getElementById('data-ger').textContent=maxTs?'Atualizado em '+new Date(maxTs).toLocaleDateString('pt-BR'):'Gerado em '+new Date().toLocaleDateString('pt-BR');
