...
function popularSelects(){
  ...
  if(dEl&&ks.length>0)dEl.value=ks[ks.length-1];
}
...
function renderDatamar(){
  ...
  function getD(p){
    if(!p)return base;
    if(isMes) return base.filter(function(r){return r.ano===p.ano&&r.mes===p.mes;});
    return base.filter(function(r){return r.ano===p.ano;});
  }
  var d1=getD(p1);
  var d2=getD(p2);
  ...
  var mesVals=meses.map(function(am){
    var t;
    if(isMes){
      t=base.filter(function(r){return r.ano===am[0]&&r.mes===am[1];}).reduce(function(s,r){return s+r.teus;},0);
    } else {
      t=base.filter(function(r){return r.ano===am[0];}).reduce(function(s,r){return s+r.teus;},0);
    }
    if(t>maxMT)maxMT=t;
